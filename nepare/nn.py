...
class FeedforwardNeuralNetwork(lightning.LightningModule):
    def __init__(self, input_size: int, hidden_size: int, num_layers: int, activation: torch.nn.Module = torch.nn.ReLU, lr: float = 1e-3):
        super().__init__()
...
class NeuralPairwiseRegressor(FeedforwardNeuralNetwork):
    def __init__(self, input_size, hidden_size, num_layers, activation = torch.nn.ReLU, lr: float = 1e-4):
        super().__init__(2*input_size, hidden_size, num_layers, activation, lr)
...
class LearnedEmbeddingNeuralPairwiseRegressor(NeuralPairwiseRegressor):
    def __init__(self, embedding_module: torch.nn.Module, embedding_size, hidden_size, num_layers, activation=torch.nn.ReLU, lr: float = 1e-4):
        super().__init__(embedding_size, hidden_size, num_layers, activation, lr)
